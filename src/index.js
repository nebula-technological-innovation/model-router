 ...
 */
export async function checkProviderHealth(gatewayUrl = 'http://localhost:8090') {
  const now = Date.now();
  ...
  // Fallback: assume both providers healthy
  healthCache = {
 ...
 * Higher is better.
 */
function scoreModel(entry, taskType, config, health) {
  let score = 0;
 ...
 */
function buildFallbackChain(scored, primary) {
  return scored
 ...
 */
export function routeModel(request, config = {}) {
 ...
 */
export function routeModelSimple(request) {
